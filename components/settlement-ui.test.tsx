...
describe("정산 UI",()=>{
  ...
  it("집계, 최소 송금과 저장된 스냅샷의 분할 근거를 표시한다",()=>{render(<SettlementView state={state}/>);expect(screen.getAllByText("101 KRW")).toHaveLength(2);expect(screen.queryByText("999 KRW")).toBeNull();expect(screen.getByText("결제자 나머지").nextElementSibling?.textContent).toBe("1 KRW");expect(screen.getAllByText("보라").length).toBeGreaterThan(0)});
});
